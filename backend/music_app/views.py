        ...
        if request.user.is_admin:
            try:
                ...
                if action == 'approve':
                    ...
                    song.save()
                    # 通知用户歌曲下线
                    user = song.uploader  # 假设歌曲模型中有 uploader 字段关联上传用户
                    send_mail(
                        '歌曲审核未通过',
                        f'您上传的歌曲 {song.song_name} 审核未通过，已下线。',
...
class SongSearchView(APIView):
    def get(self, request):
        ...
        if query:
            search_vector = SearchVector('song_name', 'singer', 'lyrics')
            songs = songs.annotate(search=search_vector).filter(search=query)
        if year:
            # 假设歌曲有发布年份字段
